...
const MyTrip = () => {
    ...
    const calculateCancellationPolicy = (departureTimeStr) => {
        ...
        if (daysDifference >= 3 && daysDifference < 5) {
            return {
                cancellable: true,
                message: "A 50% refund is available since your departure is between 3 and 5 days away.",
                refundPercentage: 50
            };
        }
        ...
        if (daysDifference >= 2 && daysDifference < 3) {
            return {
                cancellable: true,
                message: "A 25% refund is available since your departure is between 2 and 3 days away.",
                refundPercentage: 25
            };
        }
    ...
    const handleSearchTicket = async (e) => {
        ...
        try {
            ...
            const flightResponse = await axios.get('http://localhost:4000/api/users/flights');
            const flight = flightResponse.data.find(f => f.id === matchedReservation.flightId);
    ...
    const handleCancelTicket = async (bookingId) => {
        // 취소 정책 계산
        const cancellationPolicy = calculateCancellationPolicy(reservationDetails.departureTimeISO);
        ...
        <div className="my-trip-container">
            <div className="trip-search-form">
                ...
                <form onSubmit={handleSearchTicket}>
                    ...
                    <button type="submit" className="btn-search">Search Booking</button>
                </form>
            </div>

            {loading && <div className="my-trip-loading">Loading booking details...</div>}
                ...
                <div className="booking-grid">
                    ...
                    >
                        <div className="ticket-header">
                            <div className="flight-number">{reservationDetails.flightNumber}</div>
                            ...
                            <div className="flight-info">
                                <div className="time-details">
                                    <div className="departure-time">
                                        ...
                                        <span className="datetime">{reservationDetails.departureDate} {reservationDetails.departureTime}</span>
                                    </div>
                                    ...
                                    <div className="seat-info">
                                        ...
                                        <span className="number">{reservationDetails.seatNumber}</span>
                                    </div>
                                </div>
                                <div className="additional-details">
                                    <div className="arrival-time">
                                        <span className="label">Arrival: </span>
                                        <span className="datetime">{reservationDetails.arrivalDate} {reservationDetails.arrivalTime}</span>
                                    </div>
                                    <div className="plane-model">{reservationDetails.planeModel}</div>
                                </div>
                            </div>
                        </div>
